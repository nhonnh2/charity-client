...
export default function WalletPage() {
  ...
  const supportedNetworks = [
    ...
    { id: "binance", name: "Binance Smart Chain", icon: "🟡", status: "available" },
  ]
  ...
  return (
    <div className="p-6 max-w-full">
      <div className="mb-6">
        ...
        <p className="text-muted-foreground">Quản lý ví blockchain và xem thông tin giao dịch cá nhân</p>
      </div>
            ...
            <CardHeader className="pb-3">
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <CardDescription>Quản lý ví và kết nối với các mạng lưới blockchain</CardDescription>
                </div>
                {isWalletConnected ? (
              ...
              {isWalletConnected ? (
                    ...
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        <div className="flex items-center gap-2">
                          ...
                          <Badge className="bg-purple-100 text-purple-700 hover:bg-purple-100">Polygon</Badge>
                        </div>
                        <div className="flex items-center gap-2 mt-1">
                          <p className="text-sm text-muted-foreground">
                            {walletData.address.substring(0, 8)}...
                            {walletData.address.substring(walletData.address.length - 8)}
                          </p>
                      ...
                      <Card key={index} className="bg-muted/50">
                        <CardContent className="p-4">
                          <div className="flex flex-col items-center text-center py-2">
                            <h4 className="text-sm font-medium text-muted-foreground">{balance.token}</h4>
                            <p className="text-2xl font-bold">{balance.amount}</p>
                            <p className="text-sm text-muted-foreground">≈ {balance.value.toLocaleString()} VNĐ</p>
                          </div>
                        </CardContent>
                      </Card>
                    ))}
                ...
                <div className="flex flex-col items-center justify-center py-8 space-y-4">
                  ...
                  <p className="text-center text-muted-foreground max-w-md">
                    Kết nối ví blockchain để đóng góp cho các chiến dịch và theo dõi giao dịch của bạn trên nền tảng
                    TrustCharity.
                  </p>
                  <Button className="mt-2 bg-green-600 hover:bg-green-700" onClick={() => setIsWalletConnected(true)}>
                    Kết nối ví
                  </Button>
                </div>
              )}
          ...
          {isWalletConnected && (
            <Card>
              <CardHeader className="pb-0">
                ...
                <CardDescription>Các giao dịch blockchain của bạn trên nền tảng TrustCharity</CardDescription>
              </CardHeader>
              <CardContent>
                <Tabs defaultValue="overview" className="mt-4" onValueChange={setActiveTab}>
                  <TabsList className="w-full">
                    ...
                    <TabsTrigger value="received" className="flex-1">
                      Nhận quỹ ({transactions.filter((tx) => tx.type === "received").length})
                    </TabsTrigger>
                  </TabsList>
                    ...
                    {currentTransactions.map((tx) => (
                      <div key={tx.id} className="flex items-center justify-between py-3 border-b">
                        <div className="flex items-center gap-3">
                          <div
                            className={`flex h-10 w-10 items-center justify-center rounded-full ${
                              tx.type === "donation" ? "bg-red-100 text-red-600" : "bg-green-100 text-green-600"
                            }`}
                          >
                            {tx.type === "donation" ? (
                          ...
                          <div>
                            <h4 className="font-medium">
                              {tx.type === "donation" ? "Đóng góp cho chiến dịch" : "Nhận quỹ từ chiến dịch"}
                            </h4>
                            <p className="text-sm text-muted-foreground">{tx.type === "donation" ? tx.to : tx.from}</p>
                            <div className="flex items-center gap-2 mt-1">
                        ...
                        <div className="text-right">
                          <div className={`font-medium ${tx.type === "donation" ? "text-red-600" : "text-green-600"}`}>
                            {tx.type === "donation" ? "-" : "+"}
        ...
        <div className="space-y-6">
          ...
          <Card>
            <CardHeader>
              ...
              <CardDescription>Kết nối với các mạng lưới blockchain khác nhau</CardDescription>
            </CardHeader>
            <CardContent className="space-y-4">
              {supportedNetworks.map((network) => (
                <div key={network.id} className="flex items-center justify-between py-2">
                  <div className="flex items-center gap-2">
                    ...
                    <div>
                      ...
                      <p className="text-xs text-muted-foreground">
                        {network.status === "connected" ? "Đã kết nối" : "Khả dụng"}
                      </p>
                    </div>
                  </div>
          ...
          <Card>
            ...
            <CardContent className="space-y-4">
              <div className="space-y-2">
                <label className="text-sm font-medium">Địa chỉ người nhận</label>
              ...
              <Button className="w-full bg-green-600 hover:bg-green-700">Gửi token</Button>
            </CardContent>
          </Card>
          ...
          <Alert className="bg-blue-50 border-blue-200">
            ...
            <AlertDescription className="text-blue-700">
              Không bao giờ chia sẻ private key của bạn. TrustCharity không lưu trữ private key và không thể khôi phục
              ví của bạn nếu bạn mất quyền truy cập.
            </AlertDescription>
          </Alert>
          ...
          <Card>
            ...
            <CardContent className="space-y-4">
              <div className="space-y-2">
                ...
                <p className="text-sm text-muted-foreground">
                  Cài đặt tiện ích mở rộng MetaMask, tạo ví, sau đó nhấp vào nút "Kết nối ví" trên trang ví điện tử.
                </p>
              </div>
              <Separator />
              <div className="space-y-2">
              ...
              <div className="space-y-2">
                ...
                <p className="text-sm text-muted-foreground">
                  Bạn có thể mua MATIC hoặc USDT từ các sàn giao dịch tiền điện tử như Binance, Coinbase, hoặc FTX.
                </p>
              </div>
            </CardContent>
            <CardFooter>
