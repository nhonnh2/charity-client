...
export default function TransactionsPage() {
  ...
  const totalDonations = transactions.filter((tx) => tx.type === "donation").reduce((sum, tx) => sum + tx.amount, 0)
  ...
  const filteredTransactions = transactions.filter(
    (tx) =>
      ...
      (tx.hash.toLowerCase().includes(searchQuery.toLowerCase()) ||
        ...
        (tx.toName && tx.toName.toLowerCase().includes(searchQuery.toLowerCase()))),
  )
      ...
      <div className="grid grid-cols-1 lg:grid-cols-4 gap-6">
        ...
        <div className="lg:col-span-3 space-y-6">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
            <Card>
              <CardContent className="p-4">
                <div className="flex flex-col items-center text-center py-2">
                  <h4 className="text-sm font-medium text-muted-foreground">Tổng giao dịch</h4>
                  <p className="text-2xl font-bold">{transactions.length}</p>
                  <p className="text-sm text-muted-foreground">Giao dịch đã xác nhận</p>
                </div>
              </CardContent>
            </Card>
            <Card>
              <CardContent className="p-4">
                <div className="flex flex-col items-center text-center py-2">
                  <h4 className="text-sm font-medium text-muted-foreground">Tổng quyên góp</h4>
                  <p className="text-2xl font-bold">{(totalDonations / 1000000).toFixed(1)} triệu</p>
            ...
            <Card>
              <CardContent className="p-4">
                <div className="flex flex-col items-center text-center py-2">
                  <h4 className="text-sm font-medium text-muted-foreground">Tổng giải ngân</h4>
                  <p className="text-2xl font-bold">{(totalDisbursements / 1000000).toFixed(1)} triệu</p>
                  <p className="text-sm text-muted-foreground">VNĐ</p>
                </div>
              </CardContent>
            </Card>
          ...
          <Card>
            <CardHeader className="pb-0">
              ...
              <CardDescription>
                Tất cả giao dịch được ghi lại trên blockchain Polygon và có thể kiểm tra
              </CardDescription>
            </CardHeader>
            <CardContent>
              <Tabs defaultValue="all" className="mt-4" onValueChange={setActiveTab}>
                <TabsList className="w-full">
                  ...
                  <TabsTrigger value="donations" className="flex-1">
                    Đóng góp ({transactions.filter((tx) => tx.type === "donation").length})
                  </TabsTrigger>
                  <TabsTrigger value="disbursements" className="flex-1">
                    Giải ngân ({transactions.filter((tx) => tx.type === "disbursement").length})
                  </TabsTrigger>
                  <TabsTrigger value="confirmations" className="flex-1">
                    Xác nhận ({transactions.filter((tx) => tx.type === "phase_confirmation").length})
                  </TabsTrigger>
                </TabsList>
                    ...
                    <table className="w-full">
                      <thead>
                        <tr className="border-b">
                          <th className="px-4 py-3 text-left text-sm font-medium text-muted-foreground">Loại</th>
                          <th className="px-4 py-3 text-left text-sm font-medium text-muted-foreground">Từ / Đến</th>
                          <th className="px-4 py-3 text-left text-sm font-medium text-muted-foreground">Số tiền</th>
                          <th className="px-4 py-3 text-left text-sm font-medium text-muted-foreground">Thời gian</th>
                          <th className="px-4 py-3 text-left text-sm font-medium text-muted-foreground">Chiến dịch</th>
                          <th className="px-4 py-3 text-left text-sm font-medium text-muted-foreground">Mạng</th>
                            ...
                            <td className="px-4 py-3">
                              <div className="space-y-1">
                                ...
                                {tx.toName && (
                                  <>
                                    <div className="flex items-center gap-1">
                                      <span className="text-xs font-medium">Đến:</span>
                                      <span className="text-xs">{tx.toName}</span>
                                    </div>
                                    <div className="flex items-center gap-1">
                            ...
                            <td className="px-4 py-3">
                              {tx.amount > 0 ? (
                                <div className="text-sm">
                                  <span className="font-medium">{tx.amount.toLocaleString()} VNĐ</span>
                                  <span className="text-xs text-muted-foreground block">
                                    {tx.token && `(${tx.amount / 25000} ${tx.token})`}
                                  </span>
                                </div>
                              ) : (
                            ...
                            <td className="px-4 py-3">
                              <div className="flex items-center gap-2">
                                <Button variant="outline" size="sm" className="h-8" asChild>
                                  <Link href={`https://polygonscan.com/tx/${tx.hash}`} target="_blank">
        ...
        <div className="space-y-6">
            ...
            <CardContent className="space-y-4">
              <div className="space-y-2">
                ...
                <p className="text-sm text-muted-foreground">
                  Mỗi giao dịch có một hash duy nhất và được xác nhận bởi mạng Polygon. Bạn có thể kiểm tra chi tiết
                  giao dịch trên Polygon Explorer.
                </p>
              </div>
              <div className="space-y-2">
                ...
                <ul className="text-sm text-muted-foreground space-y-1">
                  ...
                  <li className="flex items-center gap-2">
                    ...
                    <span>Giải ngân: Quỹ được giải ngân cho người tạo chiến dịch</span>
                  </li>
                  <li className="flex items-center gap-2">
                    <div className="flex h-5 w-5 items-center justify-center rounded-full bg-purple-100 text-purple-600">
                      <Check className="h-3 w-3" />
                    </div>
                    <span>Xác nhận giai đoạn: Giai đoạn chiến dịch được xác nhận</span>
                  </li>
                </ul>
              </div>
            </CardContent>
          ...
          <Alert className="bg-blue-50 border-blue-200">
            ...
            <AlertTitle className="text-blue-800">Minh bạch blockchain</AlertTitle>
