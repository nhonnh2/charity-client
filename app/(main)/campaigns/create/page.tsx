...
export default function CreateCampaignPage() {
  ...
  const [campaignType, setCampaignType] = useState<'regular' | 'emergency'>('regular');
  ...
  const addPhase = () => {
    ...
    setPhases([
      ...phases,
      { id: newId, title: '', description: '', budget: 0, duration: '', documents: [] },
    ]);
  };
              ...
              <Tabs value={`step-${activeStep}`} className="w-full">
                <TabsContent value="step-1" className="m-0 p-6 space-y-6">
                  ...
                  <div className="space-y-4">
                    ...
                    <RadioGroup
                      value={campaignType}
                      onValueChange={(value: 'regular' | 'emergency') => setCampaignType(value)}
                      className="space-y-3"
                    >
                      <div className="flex items-start space-x-3 p-4 border rounded-lg hover:bg-muted/50">
                        ...
                        <div className="space-y-1 flex-1">
                          <Label htmlFor="regular" className="font-medium flex items-center">
                            <Clock className="h-4 w-4 mr-2 text-blue-600" />
                            Chiến dịch thông thường
                          </Label>
                          <p className="text-sm text-muted-foreground">
                            Chiến dịch có thể chia nhiều giai đoạn, thời gian thực hiện dài hạn
                          </p>
                        </div>
                      </div>

                      <div className={`flex items-start space-x-3 p-4 border rounded-lg hover:bg-muted/50 ${!canCreateEmergency ? 'opacity-50 cursor-not-allowed' : ''}`}>
                        <RadioGroupItem
                          value="emergency"
                          id="emergency"
                          className="mt-1"
                        ...
                        />
                        <div className="space-y-1 flex-1">
                          <Label htmlFor="emergency" className={`font-medium flex items-center ${!canCreateEmergency ? 'text-muted-foreground' : ''}`}>
                          ...
                          <p className="text-sm text-muted-foreground">
                            Chỉ 1 giai đoạn, giải ngân nhanh. Giới hạn: {new Intl.NumberFormat('vi-VN').format(maxEmergencyAmount)} VNĐ
                          </p>
                          {!canCreateEmergency && (
                  ...
                  <div className="space-y-4">
                    ...
                    <RadioGroup
                      value={fundingType}
                      onValueChange={(value: 'fixed' | 'flexible') => setFundingType(value)}
                      className="space-y-3"
                    >
                      <div className="flex items-start space-x-3 p-4 border rounded-lg hover:bg-muted/50">
                        <RadioGroupItem value="fixed" id="fixed" className="mt-1" />
                        <div className="space-y-1 flex-1">
                          <Label htmlFor="fixed" className="font-medium flex items-center">
                            <Target className="h-4 w-4 mr-2 text-green-600" />
                            Mục tiêu cố định (Fixed Funding)
                          </Label>
                          <p className="text-sm text-muted-foreground">
                            Phải đạt đủ mục tiêu mới được giải ngân. Nếu không đạt, hoàn lại toàn bộ tiền cho nhà tài trợ
                          </p>
                        </div>
                      </div>
                      ...
                      <div className="flex items-start space-x-3 p-4 border rounded-lg hover:bg-muted/50">
                        ...
                        <div className="space-y-1 flex-1">
                          <Label htmlFor="flexible" className="font-medium flex items-center">
                            <RefreshCw className="h-4 w-4 mr-2 text-blue-600" />
                            Mục tiêu linh hoạt (Flexible Funding)
                          </Label>
                          <p className="text-sm text-muted-foreground">
                            Có thể sử dụng số tiền đã quyên góp dù chưa đạt mục tiêu. Cần điều chỉnh kế hoạch và xét duyệt lại
                          </p>
                        </div>
                      </div>
                    </RadioGroup>
                  ...
                  <div className="space-y-2">
                    ...
                    <Input
                      ...
                      max={campaignType === 'emergency' ? maxEmergencyAmount : undefined}
                    />
                    {campaignType === 'emergency' && (
                      <div className="flex items-center space-x-2 p-3 bg-orange-50 border border-orange-200 rounded-lg">
                        <AlertTriangle className="h-4 w-4 text-orange-600" />
                        <p className="text-sm text-orange-700">
                          Chiến dịch khẩn cấp giới hạn tối đa {new Intl.NumberFormat('vi-VN').format(maxEmergencyAmount)} VNĐ dựa trên điểm uy tín của bạn ({userReputation} điểm)
                        </p>
                      </div>
                    )}
                    {fundingType === 'flexible' && (
                      <div className="flex items-center space-x-2 p-3 bg-blue-50 border border-blue-200 rounded-lg">
                        ...
                        <p className="text-sm text-blue-700">
                          Với mục tiêu linh hoạt, bạn có thể sử dụng số tiền đã quyên góp ngay cả khi chưa đạt mục tiêu
                        </p>
                      </div>
                    )}
                  </div>
                ...
                <TabsContent value="step-2" className="m-0 p-6 space-y-6">
                  {campaignType === 'emergency' ? (
                    <Alert>
                      ...
                      <AlertDescription>
                        Chiến dịch khẩn cấp chỉ có 1 giai đoạn duy nhất để giải ngân nhanh chóng.
                        Vui lòng cung cấp đầy đủ thông tin và tài liệu chứng minh tình trạng khẩn cấp.
                      </AlertDescription>
                    </Alert>
                  ) : (
                    <Alert>
                  ...
                  <div className="space-y-4">
                    {(campaignType === 'emergency' ? phases.slice(0, 1) : phases).map((phase, index) => (
                      <Card key={phase.id}>
                        <CardHeader className="pb-2">
                          <div className="flex items-center justify-between">
                            <CardTitle className="text-base">
                              {campaignType === 'emergency' ? 'Thông tin chiến dịch khẩn cấp' : `Giai đoạn ${index + 1}`}
                            </CardTitle>
                            {phases.length > 1 && campaignType !== 'emergency' && (
                              <Button
                                variant="ghost"
                                size="sm"
                        ...
                        <CardContent className="space-y-4 pt-0">
                          <div className="space-y-2">
                            <label className="text-sm font-medium">
                              {campaignType === 'emergency' ? 'Tiêu đề khẩn cấp' : 'Tiêu đề giai đoạn'}
                            </label>
                            <Input
                              placeholder={campaignType === 'emergency' ? 'Ví dụ: Hỗ trợ khẩn cấp cho nạn nhân lũ lụt' : 'Ví dụ: Xây dựng móng trường học'}
                              value={phase.title}
                              onChange={(e) => {
                          ...
                          <div className="space-y-2">
                            <label className="text-sm font-medium">
                              {campaignType === 'emergency' ? 'Mô tả tình trạng khẩn cấp' : 'Mô tả giai đoạn'}
                            </label>
                            <Textarea
                              placeholder={campaignType === 'emergency' ? 'Mô tả chi tiết tình trạng khẩn cấp và lý do cần hỗ trợ ngay...' : 'Mô tả chi tiết công việc trong giai đoạn này...'}
                          ...
                          <div className="space-y-2">
                            <label className="text-sm font-medium">
                              {campaignType === 'emergency' ? 'Tài liệu chứng minh khẩn cấp' : 'Tài liệu giai đoạn'}
                            </label>
                            <div className="space-y-3">
                              <div className="border-2 border-dashed rounded-lg p-4 text-center">
                                <FileText className="h-6 w-6 mx-auto text-muted-foreground mb-2" />
                                <p className="text-sm font-medium">
                                  {campaignType === 'emergency' ? 'Tải lên giấy tờ chứng minh tình trạng khẩn cấp' : 'Tải lên tài liệu kế hoạch, báo giá, giấy phép...'}
                                </p>
                              ...
                              {campaignType === 'emergency' && (
                                <div className="text-xs text-orange-600 bg-orange-50 p-2 rounded">
                                  <strong>Lưu ý:</strong> Chiến dịch khẩn cấp cần cung cấp tài liệu chứng minh tính khẩn cấp (báo cáo y tế, giấy xác nhận thiên tai, v.v.)
                                </div>
                              )}
                            </div>
                          </div>
                          ...
                          <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
                            <div className="space-y-2">
                              ...
                              <Input
                                ...
                                onChange={(e) => {
                                  const newPhases = [...phases];
                                  newPhases[index].budget = Number.parseInt(e.target.value) || 0;
                                  setPhases(newPhases);
                                }}
                              />
                            </div>
                            <div className="space-y-2">
                              <label className="text-sm font-medium">
                                {campaignType === 'emergency' ? 'Thời gian cần hỗ trợ' : 'Thời gian dự kiến'}
                              </label>
                              <Input
                                placeholder={campaignType === 'emergency' ? 'Ví dụ: Ngay lập tức' : 'Ví dụ: 30 ngày'}
                ...
                <TabsContent value="step-3" className="m-0 p-6 space-y-6">
                  <Alert>
                    ...
                    <AlertDescription>
                      Để đảm bảo tính minh bạch, chiến dịch sẽ được đội ngũ chuyên nghiệp duyệt trước khi công khai.
                      Bạn cần xác minh danh tính và đóng phí duyệt để khuyến khích người duyệt làm việc chất lượng.
                    </AlertDescription>
                  </Alert>
                  ...
                  <Card className="border-2 border-primary/20">
                    <CardHeader>
                      ...
                      <CardDescription>
                        Phí duyệt sẽ được sử dụng để trả cho đội ngũ kiểm duyệt chất lượng cao
                      </CardDescription>
                    </CardHeader>
                    <CardContent className="space-y-4">
                      ...
                      <div className="space-y-3">
                        <label className="text-sm font-medium">Điều chỉnh phí duyệt</label>
                        <div className="flex items-center space-x-4">
                          <Button
                            variant="outline"
                            size="sm"
                            onClick={() => setReviewFee(Math.max(50000, reviewFee - 10000))}
                          >
                            -10K
                          </Button>
                          <Input
                            type="number"
                            value={reviewFee}
                            onChange={(e) => setReviewFee(Math.max(50000, Number.parseInt(e.target.value) || 50000))}
                            min={50000}
                            step={10000}
                      ...
                      <Alert>
                        ...
                        <AlertDescription className="text-sm">
                          Phí duyệt sẽ được hoàn lại 100% nếu chiến dịch bị từ chối do lý do không hợp lệ.
                          Nếu chiến dịch được duyệt, phí sẽ được chuyển cho người duyệt.
                        </AlertDescription>
                      </Alert>
                    </CardContent>
                  </Card>
            ...
            <CardFooter className="border-t px-6 py-4 flex justify-between">
              ...
              {activeStep < 3 ? (
                ...
                <Button className="bg-green-600 hover:bg-green-700">
                  Tạo chiến dịch & Đóng phí duyệt ({new Intl.NumberFormat('vi-VN').format(reviewFee)} VNĐ)
                </Button>
              )}
            </CardFooter>
          </Card>
            ...
            <CardContent className="space-y-4">
              <div className="space-y-2">
                ...
                <p className="text-sm text-muted-foreground pl-7">
                  Chọn loại chiến dịch (thông thường/khẩn cấp), mục tiêu funding và thông tin chi tiết.
                </p>
              </div>
              <div className="space-y-2">
                <h3 className="font-medium flex items-center">
                ...
                <p className="text-sm text-muted-foreground pl-7">
                  Chia giai đoạn (thông thường) hoặc cung cấp thông tin khẩn cấp với tài liệu chứng minh.
                </p>
              </div>
              <div className="space-y-2">
                <h3 className="font-medium flex items-center">
          ...
          <Card>
            ...
            <CardContent className="space-y-4">
              <Alert>
                ...
                <AlertDescription className="text-sm">
                  Sau khi tạo chiến dịch, trạng thái sẽ là <strong>"Đang chờ duyệt"</strong>.
                  Đội ngũ duyệt sẽ kiểm tra và phản hồi trong vòng 24-72 giờ.
                </AlertDescription>
              </Alert>
              ...
              <div className="space-y-2">
                ...
                <p className="text-xs text-muted-foreground pl-6">
                  Fixed: Phải đạt 100% mục tiêu mới giải ngân.
                  Flexible: Có thể dùng tiền dù chưa đạt mục tiêu.
                </p>
              </div>
              ...
              <div className="space-y-2">
                ...
                <p className="text-xs text-muted-foreground pl-6">
                  Phí duyệt đảm bảo chất lượng kiểm duyệt và khuyến khích người duyệt làm việc tận tình. Phí sẽ được hoàn lại nếu chiến dịch bị từ chối không hợp lý.
                </p>
              </div>
              ...
              <div className="space-y-2">
                ...
                <p className="text-xs text-muted-foreground pl-6">
                  Hoàn thành các chiến dịch đúng tiến độ, cập nhật thường xuyên và minh bạch trong chi tiêu.
                </p>
              </div>
              <div className="space-y-2">
                <h3 className="text-sm font-medium flex items-center">
                ...
                <p className="text-xs text-muted-foreground pl-6">
                  Mỗi giai đoạn sẽ được giải ngân sau khi bạn cập nhật tiến độ và được 51% người đóng góp xác nhận.
                </p>
              </div>
              <div className="space-y-2">
                <h3 className="text-sm font-medium flex items-center">
                ...
                <p className="text-xs text-muted-foreground pl-6">
                  Nền tảng thu 2.5% phí trên mỗi giao dịch thành công để duy trì hệ thống.
                </p>
              </div>
            </CardContent>
            <CardFooter>
