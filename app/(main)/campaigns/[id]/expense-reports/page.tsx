...
export default function ExpenseReportsPage({
    ...
    const stageReports = [
        {
            ...
            documents: [
                ...
                { name: 'Báo cáo chi tiêu giai đoạn 1.xlsx', size: '1.1MB', type: 'excel' },
            ...
            description: 'Chi tiêu cho giai đoạn đào móng, đổ bê tông và xây dựng khung chính. Tiết kiệm được 500.000 VNĐ so với dự kiến nhờ thương lượng giá vật liệu tốt.',
        },
        {
            stage: 2,
            ...
            completedAt: null,
            documents: [
                { name: 'Hóa đơn vật liệu giai đoạn 2.pdf', size: '3.1MB', type: 'pdf' },
                { name: 'Báo cáo chi tiêu chi tiết.xlsx', size: '1.5MB', type: 'excel' },
            ],
            expenseCategories: [
            ...
            description: 'Chi tiêu vượt dự kiến 1.5M VNĐ do giá vật liệu tăng. Đã sử dụng 500K từ tiết kiệm giai đoạn 1, cần bổ sung thêm 1M từ quỹ dự phòng.',
        },
        {
            stage: 3,
            ...
            documents: [],
            expenseCategories: [],
            description: 'Giai đoạn này sẽ được mở khóa sau khi giai đoạn 2 được phê duyệt chứng minh chi tiêu.',
        },
    ];
    ...
    const totalBudget = stageReports.reduce((sum, stage) => sum + stage.budget, 0);
    const totalSpent = stageReports.reduce((sum, stage) => sum + stage.actualSpent, 0);
    const totalSurplus = stageReports.reduce((sum, stage) => sum + stage.surplus, 0);
    const totalDeficit = stageReports.reduce((sum, stage) => sum + stage.deficit, 0);
                    ...
                    <CardContent className="p-4">
                        <div className="flex items-center justify-between">
                            <div>
                                ...
                                <p className="text-2xl font-bold text-green-600">+{(totalSurplus / 1000).toFixed(0)}K</p>
                            </div>
                    ...
                    <CardContent className="p-4">
                        <div className="flex items-center justify-between">
                            <div>
                                ...
                                <p className="text-2xl font-bold text-red-600">-{(totalDeficit / 1000).toFixed(0)}K</p>
                            </div>
                            ...
                            <Tabs defaultValue="overview" className="w-full">
                                ...
                                <TabsContent value="overview" className="space-y-4">
                                    <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                                        ...
                                        <div className="space-y-1">
                                            <p className="text-sm text-muted-foreground">Chi tiêu thực tế</p>
                                            <p className="text-lg font-semibold">
                                        ...
                                        <div className="space-y-1">
                                            ...
                                            <p className={`text-lg font-semibold ${report.surplus > 0 ? 'text-green-600' :
                                                    report.deficit > 0 ? 'text-red-600' : 'text-gray-600'
                                                }`}>
                                                {report.surplus > 0 ? `+${report.surplus.toLocaleString()}` :
                                                    report.deficit > 0 ? `-${report.deficit.toLocaleString()}` : '0'} VNĐ
                                            </p>
                                        </div>
                                        <div className="space-y-1">
                                    ...
                                    {report.status === 'pending' && (
                                        <Alert className="border-orange-200 bg-orange-50">
                                            ...
                                            <AlertDescription>
                                                Báo cáo chi tiêu đang chờ duyệt. Thời gian duyệt dự kiến: 3-5 ngày làm việc.
                                            </AlertDescription>
                                        </Alert>
                                    )}
                                    ...
                                    {report.status === 'locked' && (
                                        <Alert className="border-gray-200 bg-gray-50">
                                            ...
                                            <AlertDescription>
                                                Giai đoạn này chưa được mở khóa. Vui lòng hoàn thành chứng minh chi tiêu giai đoạn trước.
                                            </AlertDescription>
                                        </Alert>
                                    )}
                                </TabsContent>
                                ...
                                <TabsContent value="breakdown" className="space-y-4">
                                    {report.expenseCategories.length > 0 ? (
                                        <div className="space-y-4">
                                            <h4 className="font-medium">Phân bổ chi tiêu theo danh mục</h4>
                                    ...
                                    <div className="flex items-center space-x-4 text-sm text-muted-foreground">
                                        {report.submittedAt && (
                                            <div className="flex items-center space-x-1">
                                                ...
                                                <span>Nộp: {new Date(report.submittedAt).toLocaleDateString('vi-VN')}</span>
                                            </div>
                                        )}
                                        {report.approvedAt && (
                                            <div className="flex items-center space-x-1">
                                                <CheckCircle className="h-4 w-4" />
                                                <span>Duyệt: {new Date(report.approvedAt).toLocaleDateString('vi-VN')}</span>
                                            </div>
                                        )}
                                    </div>
