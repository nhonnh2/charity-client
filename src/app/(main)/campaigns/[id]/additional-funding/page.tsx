...
export default function AdditionalFundingPage({
  ...
  const additionalFundings = [
    {
      ...
      description:
        'Cần thêm kinh phí để mua máy tính và thiết bị học tập hiện đại cho các em học sinh.',
      progress: 62.5,
    },
    {
      id: 2,
      ...
      description:
        'Xây dựng thêm một phòng thư viện nhỏ để phục vụ việc đọc sách của học sinh.',
      progress: 100,
    },
    {
      id: 3,
  ...
  const getStatusBadge = (status: string) => {
    switch (status) {
      case 'active':
        return (
          <Badge className='bg-blue-100 text-blue-700'>Đang kêu gọi</Badge>
        );
      case 'completed':
        return (
                ...
                <div className='space-y-2'>
                  <div className='flex items-center justify-between text-sm'>
                    ...
                    <span className='text-muted-foreground'>
                      Còn lại:{' '}
                      {(funding.target - funding.raised).toLocaleString()} VNĐ
                    </span>
                  </div>
              ...
              {funding.status === 'completed' && (
                <div className='bg-green-50 p-3 rounded-lg border border-green-200'>
                  <p className='text-sm text-green-800'>
                    ✅ Hoàn thành mục tiêu {funding.target.toLocaleString()} VNĐ
                    với {funding.backers} người ủng hộ
                  </p>
                </div>
              )}
              ...
              {funding.status === 'rejected' && (
                <div className='bg-red-50 p-3 rounded-lg border border-red-200'>
                  <p className='text-sm text-red-800'>
                    ❌ Yêu cầu kêu gọi này đã bị từ chối do không đáp ứng tiêu
                    chí cộng đồng
                  </p>
                </div>
              )}
